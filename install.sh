...
case $os_type in
    ...
    MINGW*|MSYS*|CYGWIN*)
        os_name="windows"
        target_dir="/c/tza-probe"  # Use C:\tza-probe on Windows
        ;;
    *)
...
if [ "$EUID" -ne 0 ] && [ "$install_dir_specified" = false ]; then
    case "$os_name" in
        linux|freebsd)
            target_dir="${XDG_DATA_HOME:-$HOME/.local/share}/tza-probe"
            ;;
    esac
fi

agent_path="${target_dir}/agent"
legacy_agent_path="/opt/komari/agent"
...
echo -e "${WHITE}===========================================${NC}"
echo -e "${WHITE}    TZA Probe Agent Installation Script     ${NC}"
echo -e "${WHITE}===========================================${NC}"
echo ""
...
uninstall_previous() {
    ...
    # Stop the legacy service during an in-place upgrade, but keep its data directory.
    ...
    if [ "$user_service" = true ]; then
        ...
        system_plist="/Library/LaunchDaemons/io.tza.probe.${service_name}.plist"
        user_plist="$HOME/Library/LaunchAgents/io.tza.probe.${service_name}.plist"
    ...
    if [ -f "$legacy_agent_path" ] && [ "$legacy_agent_path" != "$agent_path" ]; then
        log_info "保留旧 Agent 数据路径 $legacy_agent_path；新版本将安装到 $agent_path。"
    fi
}
...
file_name="tza-probe-agent-${os_name}-${arch}"
...
if [ -n "$github_proxy" ]; then
    ...
    download_url="${github_proxy}/https://github.com/leeska/TZA-Probe-Agent/releases/${download_path}/${file_name}"
else
    # Direct access to GitHub releases
    download_url="https://github.com/leeska/TZA-Probe-Agent/releases/${download_path}/${file_name}"
fi
...
if ! curl -L -o "$agent_path" "$download_url"; then
...
chmod +x "$agent_path"
if [ "$EUID" -eq 0 ] && [ "$service_user" != "root" ]; then
    chown "$service_user" "$agent_path"
fi
log_success "TZA Probe Agent installed to ${GREEN}$agent_path${NC}"
...
if [ "$init_system" = "nixos" ]; then
    ...
    echo -e "${CYAN}  description = \"TZA Probe Agent Service\";${NC}"
...
name="TZA Probe Agent Service"
description="TZA Probe monitoring agent"
command="${agent_path}"
...
EOF
    ...
    cat > "$service_file" << EOF
[Unit]
Description=TZA Probe Agent Service
...
EOF
    ...
    if [[ "$target_dir" =~ ^/Users/.* ]] || [ "$EUID" -ne 0 ]; then
        ...
        plist_file="$plist_dir/io.tza.probe.${service_name}.plist"
...
<dict>
    <key>Label</key>
    <string>io.tza.probe.${service_name}</string>
...
echo -e "${WHITE}===========================================${NC}"
if [ -f /etc/NIXOS ]; then
    log_success "TZA Probe Agent binary installed!"
    ...
    log_success "TZA Probe Agent installation completed!"
fi
