...
func runV2PullLoop(ctx context.Context) {
	for {
		...
		payload := v2.NewRequest(pullID, v2.MethodAgentPull, map[string]interface{}{
			"capabilities":  []string{"exec", "ping", "message", "event", "terminal", "file", "carrier_route"},
...
func processV2Event(conn *ws.SafeConn, method string, params interface{}, eventID string) bool {
	...
	case v2.MethodAgentCarrierRouteProbe:
		var p v2.CarrierRouteProbeParams
		if err := v2.BindParams(params, &p); err == nil {
			go runCarrierRouteProbe(conn, p)
			return true
		} else {
			log.Printf("bad v2 carrier route params: %v", err)
		}
	default:
