# Windows PowerShell installation script for TZA Probe Agent
...
# Default parameters
$InstallDir = Join-Path $Env:ProgramFiles "TZA Probe"
$ServiceName = "tza-probe-agent"
$GitHubProxy = ""
$AgentArgs = @()
$InstallVersion = ""
...
# Paths
$BinaryName = "tza-probe-agent-windows-$arch.exe"
$AgentPath = Join-Path $InstallDir "tza-probe-agent.exe"
...
if ($InstallVersion -ne "") {
    ...
    $ApiUrl = "https://api.github.com/repos/leeska/TZA-Probe-Agent/releases/latest"
    try {
...
Log-Success "Installing TZA Probe Agent version: $versionToInstall"

# Construct download URL
$BinaryName = "tza-probe-agent-windows-$arch.exe"
$DownloadUrl = if ($GitHubProxy) { "$GitHubProxy/https://github.com/leeska/TZA-Probe-Agent/releases/download/$versionToInstall/$BinaryName" } else { "https://github.com/leeska/TZA-Probe-Agent/releases/download/$versionToInstall/$BinaryName" }
...
nssm set $ServiceName DisplayName "TZA Probe Agent Service"
...
Log-Success "TZA Probe Agent installation completed!"
Log-Config "Service name: $ServiceName"
