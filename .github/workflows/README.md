...
This directory contains the GitHub Actions workflows for TZA Probe Agent. Keep this
...
- Release and snapshot assets are named `tza-probe-agent-${GOOS}-${GOARCH}`.
- Windows assets append `.exe`.
- The Dockerfile expects prebuilt Linux binaries named
  `tza-probe-agent-${TARGETOS}-${TARGETARCH}` in the Docker build context.
...
- Uploads all `tza-probe-agent-*` artifacts to that prerelease.
...
- The Dockerfile creates `/.tza-probe-agent-container`. Snapshot-aware auto-update
...
- Snapshot agents running in Docker skip binary self-update when
  `/.tza-probe-agent-container` exists.
...
- A binary running in Docker can potentially replace `/app/tza-probe-agent`, but
