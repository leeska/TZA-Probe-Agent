...
var (
	...
	Repo           string = "leeska/TZA-Probe-Agent"
)

const (
	snapshotVersionPrefix = "Snapshot-"
	containerMarkerPath   = "/.tza-probe-agent-container"
...
func expectedAssetName(goos, goarch string) string {
	name := fmt.Sprintf("tza-probe-agent-%s-%s", goos, goarch)
	if goos == "windows" {
		name += ".exe"
	}
	return name
...
func listGitHubReleases(owner, repo string) ([]githubRelease, error) {
	...
	for page := 1; ; page++ {
		...
		req.Header.Set("User-Agent", "tza-probe-agent")
