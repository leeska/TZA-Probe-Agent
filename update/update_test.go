...
func TestExpectedAssetName(t *testing.T) {
	tests := []struct {
		...
		{"linux", "amd64", "tza-probe-agent-linux-amd64"},
		{"darwin", "arm64", "tza-probe-agent-darwin-arm64"},
		{"windows", "amd64", "tza-probe-agent-windows-amd64.exe"},
	}
...
func TestSelectLatestSnapshotRelease(t *testing.T) {
	assetName := "tza-probe-agent-linux-amd64"
	...
	releases := []githubRelease{
		...
		testRelease("Snapshot-2607061600", true, false, base.Add(7*time.Hour), "tza-probe-agent-linux-arm64"),
...
func TestSelectLatestSnapshotReleaseNoMatch(t *testing.T) {
	releases := []githubRelease{
		testRelease("v1.2.3", false, false, time.Now(), "tza-probe-agent-linux-amd64"),
		testRelease("Snapshot-2607061200", true, true, time.Now(), "tza-probe-agent-linux-amd64"),
		testRelease("Snapshot-2607061300", true, false, time.Now(), "tza-probe-agent-linux-arm64"),
	}

	if got, ok := selectLatestSnapshotRelease(releases, "tza-probe-agent-linux-amd64"); ok {
